...
class Model {
    ...
    Model(int width, int height) {
        ...
        for (int i = 0; i < _height - 1; i += 1) {
            for (int j = 0; j < _width - 1; j += 1) {
                _cells[i][j] = false;
            }
        }
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i++) {
            for (int j = 0; j < piece.width(); j++) {
                if (piece.get(i, j) && get(i + row, j + col)) {
                    return false;
                }
            }
        }
    ...
    void clearFilledLines() {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < width(); j++) {
                if (i == 0 && counts[i][j] == width()) {
                    nrows += 1;
                    for (int k = 0; k < width(); k++) {
                        _cells[j][k] = false;
                    }
                } else if (i == 1 && counts[i][j] == height()) {
                    ncols += 1;
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int bonus;
        bonus = (_streakLength) * (width() * nrows + height() * ncols);
        return bonus + height() * (ncols) + (width() - ncols) * nrows;
    }
